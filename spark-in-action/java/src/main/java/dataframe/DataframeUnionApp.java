...
public class DataframeUnionApp {
    private final String PATH = "/Users/yjkim-studio/data/spark/";
    ...
    // Build the dataframe containing the Wake county restaurants
    private Dataset<Row> buildWakeRestaurantsDataframe() {
        Dataset<Row> df = this.spark.read().format("csv")
                .option("header", "true")
                .load(PATH + "Restaurants_in_Wake_County.csv");
    ...
    private Dataset<Row> buildDurhamRestaurantsDataframe() {
        Dataset<Row> df = this.spark.read().format("json")
                .load(PATH + "Restaurants_in_Durham_County_NC.json");
