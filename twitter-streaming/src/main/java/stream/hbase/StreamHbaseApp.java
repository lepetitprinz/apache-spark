...
import stream.StreamJsonSinkApp;

import static org.apache.spark.sql.functions.*;
import static org.apache.spark.sql.functions.regexp_replace;

public class StreamHbaseApp {
    private static Logger log =
            LoggerFactory.getLogger(StreamJsonSinkApp.class);

    public static void main(String[] args) {
        StreamHbaseApp app = new StreamHbaseApp();
        try {
            app.start();
    ...
    private void start() throws TimeoutException {
        SparkSession spark = SparkSession.builder()
                .appName("Twitter streaming to Hbase")
                .master("local")
                .getOrCreate();
        ...
        String catalog = "{'table':{'namespace':'default', 'name':'test'}, " +
                "" +
                "}";
